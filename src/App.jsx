...
export default function App() {
  ...
  useEffect(() => {
  (async () => {
    // Coba dapatkan session dari Supabase (online)
    const { data } = await supabase.auth.getSession();
    let sessionData = data?.session || null;

    // Jika tidak ada session dari Supabase, coba dari localStorage
    if (!sessionData) {
  ...
  // Simpan session ke localStorage saat berubah
  ...
  return () => {
   ...
   // fetchTasks();
   // fetchMessages();
   // Dalam useEffect utama
window.addEventListener('online', () => {
  // Sync data ketika kembali online
  fetchTasks();
  fetchMessages();
});
...
const fetchTasks = async () => {
  ...
  try {
    ...
    if (!error) {
      setProfile(data);
      // Simpan profile ke localStorage untuk offline use
