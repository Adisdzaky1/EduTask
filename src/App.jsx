...
const NOTIFICATION_CONFIG = {
  ...
  //10 * 60 * 1000, // 10 menit (interval pemeriksaan)
  OVERDUE_THRESHOLD: 5 * 60 * 1000,
  // 12 * 60 * 60 * 1000, // 12 jam (waktu tunggu sebelum notif)
  ACTIVE_HOURS: { start: 6, end: 21 } // Jam aktif notifikasi (6:00 - 21:00 WIB)
};

// Untuk testing, bisa diubah via console:
// NOTIFICATION_CONFIG.OVERDUE_THRESHOLD = 5 * 60 * 1000; // 5 menit
// NOTIFICATION_CONFIG.CHECK_INTERVAL = 30 * 1000; // 30 detik
...
export default function App() {
  ...
  useEffect(() => {
    // Jalankan notifikasi otomatis secara periodic
      ...
      <div className="col" style={{ marginTop: 8 }}>
        {rows.map(n => (
          ...
          >
            ...
            {n.metadata ? (
              // Tampilan untuk notifikasi tugas
              <>
              ...
              // Tampilan untuk notifikasi biasa
...
export async function overdueNotifier() {
  try {
    ...
    // Hanya jalankan antara jam aktif
    if (jakartaNow.hour() < NOTIFICATION_CONFIG.ACTIVE_HOURS.start || 
        jakartaNow.hour() >= NOTIFICATION_CONFIG.ACTIVE_HOURS.end) {
      return;
    }
    
    const threshold = dayjs().subtract(NOTIFICATION_CONFIG.OVERDUE_THRESHOLD, 'millisecond').toISOString();
    
    console.log(`Memeriksa tugas yang terlambat (threshold: ${NOTIFICATION_CONFIG.OVERDUE_THRESHOLD / (60 * 60 * 1000)} jam)`);
          ...
          if (!existingNotif) {
            ...
            const { error: insertError } = await supabase
              ...
              .insert({
                ...
                body: `Tugas "${task.title}" sudah lewat batas waktu pengumpulan.`,
                metadata: {
                  task_title: task.title,
                  task_description: task.description,
                  due_at: task.due_at,
                  category: task.categories?.name || 'Umum',
                  priority: task.priority,
                  class_name: task.classes?.name || 'Kelas Tidak Diketahui'
                }
              });
            ...
            if (insertError) {
              ...
              console.log(`Notifikasi terkirim ke ${student.username} untuk tugas ${task.title}`);
            }
          }
        }
      }
    ...
    console.error('Error dalam overdueNotifier:', e);
  }
}
