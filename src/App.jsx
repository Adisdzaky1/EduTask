...
const fetchTasks = async () => {
  ...
  try {
    ...
    if (error) {
      ...
      // Jika profil tidak ditemukan, coba buat profil admin
      if (error.code === 'PGRST116') { // Code untuk "not found"
        const { data: userData } = await supabase.auth.getUser();
        if (userData.user && userData.user.email === 'admin@edutask.local') {
          return await createAdminProfile(userData.user);
        }
      }
