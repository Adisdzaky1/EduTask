...
function OwnerDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { count: tasksCount } = await supabase.from('tasks').select('*', { count: 'exact' });
      const { count: classesCount } = await supabase.from('classes').select('*', { count: 'exact' });
      const { count: notifCount } = await supabase.from('notifications').select('*', { count: 'exact' });
      const { count: submissionsCount } = await supabase.from('submissions').select('*', { count: 'exact' });
  ...
  const setupRealtime = () => {
    const channels = [
      supabase.channel('profiles_channel').on('postgres_changes', { event: '*', schema: 'public', table: 'profiles' }, loadData),
      supabase.channel('tasks_channel').on('postgres_changes', { event: '*', schema: 'public', table: 'tasks' }, loadData),
      supabase.channel('classes_channel').on('postgres_changes', { event: '*', schema: 'public', table: 'classes' }, loadData),
      supabase.channel('notifications_channel').on('postgres_changes', { event: '*', schema: 'public', table: 'notifications' }, loadData),
      supabase.channel('submissions_channel').on('postgres_changes', { event: '*', schema: 'public', table: 'submissions' }, loadData)
    ];
  ...
  const runOverdueNotifier = async () => {
    try {
      ...
      alert('Gagal menjalankan notifikasi terlambat');
    }
  };
    ...
    <div className="col">
      <div className="card pop" style={{padding: '16px'}}>
        ...
        <div className="row" style={{
          ...
          <button 
            className={`btn ghost ${activeTab === 'classes' ? 'active' : ''}`}
          ...
          <button 
            className={`btn ghost ${activeTab === 'notifications' ? 'active' : ''}`}
          ...
          <button 
            className={`btn ghost ${activeTab === 'tools' ? 'active' : ''}`}
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-2" style={{gap: '12px'}}>
            <div className="card" style={{padding: '16px', textAlign: 'center'}}>
              <div className="title" style={{fontSize: '14px', marginBottom: '8px'}}>Total Pengguna</div>
              <div style={{fontSize: '24px', fontWeight: 'bold', color: '#6c8bff'}}>{stats.users}</div>
            </div>
            <div className="card" style={{padding: '16px', textAlign: 'center'}}>
              <div className="title" style={{fontSize: '14px', marginBottom: '8px'}}>Total Tugas</div>
              <div style={{fontSize: '24px', fontWeight: 'bold', color: '#ffab00'}}>{stats.tasks}</div>
            </div>
            <div className="card" style={{padding: '16px', textAlign: 'center'}}>
              <div className="title" style={{fontSize: '14px', marginBottom: '8px'}}>Total Kelas</div>
              <div style={{fontSize: '24px', fontWeight: 'bold', color: '#7ef7d1'}}>{stats.classes}</div>
            </div>
            <div className="card" style={{padding: '16px', textAlign: 'center'}}>
              <div className="title" style={{fontSize: '14px', marginBottom: '8px'}}>Total Notifikasi</div>
              <div style={{fontSize: '24px', fontWeight: 'bold', color: '#ff6b6b'}}>{stats.notifications}</div>
            </div>
            <div className="card" style={{padding: '16px', textAlign: 'center'}}>
              <div className="title" style={{fontSize: '14px', marginBottom: '8px'}}>Total Pengumpulan</div>
              <div style={{fontSize: '24px', fontWeight: 'bold', color: '#9f7aea'}}>{stats.submissions}</div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'notifications' && (
          <div className="col">
            ...
            <button className="btn" onClick={sendNotification} style={{marginBottom: '20px'}}>
              Kirim Notifikasi
            </button>
        ...
        {activeTab === 'tools' && (
          <div className="col">
            ...
            <div className="card" style={{padding: '16px', marginBottom: '16px'}}>
              ...
              <button className="btn" onClick={runOverdueNotifier}>
