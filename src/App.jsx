...
export default function App() {
  ...
  // ============================
  useEffect(() => {
    // Konfigurasi threshold waktu untuk notifikasi (dalam milidetik)
    // Default: 12 jam (12 * 60 * 60 * 1000)
    //window.OVERDUE_THRESHOLD_MS = 12 * 60 * 60 * 1000;
    
    // Untuk testing, bisa diubah melalui console browser:
     window.OVERDUE_THRESHOLD_MS = 5 * 60 * 1000; // 5 menit untuk testing
    
    // Jalankan notifikasi otomatis secara periodic setiap 10 menit
    ...
    return () => clearInterval(notifierInterval);
  }, []);
  
  // Fungsi untuk menangani ketika notifikasi di-klik
  const handleTaskSelectFromNotification = (taskId) => {
    setTab("tasks");
...
// -----------------------------------------------------
function NotificationsPage({ profile, dayjs, onTaskSelect }){
  ...
  const handleNotificationClick = (notification) => {
    if (notification.task_id) {
      onTaskSelect(notification.task_id);
    }
  };
    ...
    <div className="card pop" style={{padding:12}}>
      ...
      <div className="col" style={{marginTop:8}}>
        {rows.map(n => (
          <div 
            key={n.id} 
            className={`item ${n.task_id ? 'card' : ''}`} 
            style={{
              cursor: n.task_id ? 'pointer' : 'default',
              border: n.task_id ? '1px solid rgba(255, 255, 255, 0.12)' : 'none',
              padding: n.task_id ? '16px' : '12px',
              borderRadius: n.task_id ? '14px' : '8px'
            }}
            onClick={() => n.task_id && handleNotificationClick(n)}
          >
            <div className="small muted">{dayjs(n.created_at).format('DD MMM HH:mm')}</div>
            <div style={{fontWeight:600, marginBottom: n.task_id ? '8px' : '4px'}}>{n.title}</div>
            
            {n.task_id ? (
              // Tampilan card untuk notifikasi tugas
              <>
                <div style={{marginBottom: '8px'}}>{n.metadata?.task_title || n.body}</div>
                ...
                <div className="row" style={{gap: '6px', flexWrap: 'wrap'}}>
                  <span className="badge" style={{
                    ...
                    {n.metadata?.category || 'Umum'}
                  </span>
                  
                  <span className="badge" style={{
                    background: n.metadata?.priority === 'high' ? 'rgba(255, 107, 107, 0.12)' : 
                               n.metadata?.priority === 'normal' ? 'rgba(255, 171, 0, 0.12)' : 'rgba(126, 247, 209, 0.12)',
                    borderColor: n.metadata?.priority === 'high' ? 'rgba(255, 107, 107, 0.25)' : 
                                n.metadata?.priority === 'normal' ? 'rgba(255, 171, 0, 0.25)' : 'rgba(126, 247, 209, 0.25)',
                    color: n.metadata?.priority === 'high' ? '#ff6b6b' : 
                          n.metadata?.priority === 'normal' ? '#ffab00' : '#7ef7d1',
                    fontSize: '11px',
                    padding: '4px 8px'
                  }}>
                    {n.metadata?.priority === 'high' ? 'Tinggi' : 
                     n.metadata?.priority === 'normal' ? 'Normal' : 'Rendah'}
                  </span>
                  ...
                  {n.metadata?.due_at && (
                    <span className="badge" style={{
                      ...
                      Due: {dayjs(n.metadata.due_at).format('DD MMM HH:mm')}
                    </span>
                  )}
                </div>
                
                <div className="small muted" style={{marginTop: '8px'}}>
                  Klik untuk melihat detail tugas
                </div>
              </>
            ) : (
              // Tampilan biasa untuk notifikasi lainnya
...
export async function overdueNotifier(){ 
  try{ 
    ...
    // Hanya jalankan antara jam 06:00-21:00 WIB
    if (jakartaNow.hour() < 6 || jakartaNow.hour() >= 21) return;
    
    // Gunakan variabel konfigurasi untuk threshold waktu (dalam milidetik)
    const OVERDUE_THRESHOLD_MS = window.OVERDUE_THRESHOLD_MS || (5 * 60 * 1000); // window.OVERDUE_THRESHOLD_MS || (12 * 60 * 60 * 1000); // Default 12 jam
    const threshold = dayjs().subtract(OVERDUE_THRESHOLD_MS, 'millisecond').toISOString();
    
    console.log(`Memeriksa tugas yang terlambat (threshold: ${OVERDUE_THRESHOLD_MS / (60 * 60 * 1000)} jam)`);
    ...
    for(const t of (tasks||[])){ 
      ...
      for(const s of (students||[])){ 
        // Periksa apakah siswa sudah mengumpulkan
        const { data: subs } = await supabase.from('submissions')
          .select('id')
          .eq('task_id', t.id)
          .eq('student_id', s.id);
        
        // Jika belum ada pengumpulan, kirim notifikasi
        if(!subs || subs.length === 0){ 
          ...
          if (!existingNotif) {
            // Simpan notifikasi dengan tipe khusus dan data tugas
            await supabase.from('notifications').insert({ 
              user_id: s.id, 
              task_id: t.id,
              type: 'overdue',
              title: 'Tugas Terlambat', 
              body: `Tugas "${t.title}" sudah lewat batas waktu pengumpulan.`,
              metadata: {
                task_title: t.title,
                task_description: t.description,
                due_at: t.due_at,
                category: t.categories?.name || 'Umum',
                priority: t.priority
              }
            }); 
            
            console.log(`Notifikasi terkirim ke ${s.username} untuk tugas ${t.title}`);
          }
        } 
      } 
    } 
